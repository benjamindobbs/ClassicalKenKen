...
// Call initDailyProgress(type) after sign-in. type: 'kenken' | 'sat'
...
async function refreshDailyProgress(type) {
    if (typeof localMode !== 'undefined' && localMode) return;
    const prevCount = _dpData?.today?.[type] ?? 0;
    try {
        const res = await authFetch('/api/student/daily-progress');
        if (!res.ok) return;
        _dpData = await res.json();
        const newCount = _dpData?.today?.[type] ?? 0;
        _renderPill(type);
...
function _renderPill(type) {
    ...
    const relevant = act === type || act === 'both' || act === 'either';
    ...
    const done  = Math.min(_dpData.today[type] ?? 0, required);
